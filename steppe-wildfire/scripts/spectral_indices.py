"""NDVI/NBR для расширенного 5-канального входа модели."""
...
def nbr(nir: np.ndarray, swir2: np.ndarray) -> np.ndarray:
    """NBR = (NIR - SWIR2) / (NIR + SWIR2). Диапазон [-1, 1]."""
    nir = nir.astype(np.float32)
...
def build_extended_input(swir2: np.ndarray, swir1: np.ndarray, nir: np.ndarray,
    ...
    """[SWIR2, SWIR1, NIR, NDVI, NBR] -> массив (5, H, W) для WildfireUNet(in_channels=5)."""
...
if __name__ == "__main__":
    cases = {
        "сухая степь":     dict(swir2=0.25, swir1=0.30, nir=0.35, red=0.28),
        "активный огонь":  dict(swir2=0.75, swir1=0.65, nir=0.30, red=0.20),
        "нагретый солончак": dict(swir2=0.40, swir1=0.42, nir=0.45, red=0.40),
    }

    for name, bands in cases.items():
        arrs = {k: np.full((4, 4), v) for k, v in bands.items()}
        stacked = build_extended_input(**arrs)
        print(f"{name:18s} -> NDVI={stacked[3,0,0]:.3f}  NBR={stacked[4,0,0]:.3f}")
