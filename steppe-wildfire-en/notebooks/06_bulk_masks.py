"""
ONE CELL: generates masks for ALL cases in FIRE_CASES at once. Run AFTER
all GEE export tasks are COMPLETED. Skips cases whose GeoTIFF isn't on
Drive yet instead of crashing.

Paste this whole thing into Colab and run.
"""
...
BBOX_DEG = 0.05  # buffer around the point in degrees (~matches the 5km radius from step 01)
...
# This must match FIRE_CASES in 01_gee_download.py
FIRE_CASES = [
...
for name, date_str, lon, lat in FIRE_CASES:
    ...
    if not os.path.exists(tif_path):
        print(f"{name}: SKIPPED - file not ready yet (check GEE Tasks)")
        failed.append(name)
        continue
    try:
        ...
        print(f"{name}: OK, image {arr.shape}, FIRMS points={len(points)}, "
              f"fire pixels={mask.sum()}/{mask.size}")
        ok.append(name)
    except Exception as e:
        print(f"{name}: ERROR - {e}")
        failed.append(name)

print(f"\nTotal: succeeded {len(ok)}/{len(FIRE_CASES)}, skipped/failed: {len(failed)}")
if failed:
    print("Not processed:", failed)

print("\nNext: python scripts/train.py --data-dir data --in-channels 3 --batch-size 4 --epochs 30")
