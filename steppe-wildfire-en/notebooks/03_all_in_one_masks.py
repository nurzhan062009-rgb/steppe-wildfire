"""
ONE CELL: generate fire masks via NASA FIRMS for a couple of cases.
Paste this whole thing into Colab and run. Assumes:
  - GeoTIFFs are already in /content/drive/MyDrive/steppe_wildfire/
  - Google Drive is already mounted (drive.mount('/content/drive'))
"""
...
for name, date_str, bbox in cases_info:
    ...
    print(f"{name}: image saved, shape {arr.shape}")

    points = fetch_firms_points(*bbox, date_str=date_str)
    print(f"{name}: FIRMS points = {len(points)}")

    mask = rasterize_fire_mask(points, tif_path)
    np.save(f"data/masks/{name}.npy", mask.astype(np.uint8))
    print(f"{name}: mask saved, fire pixels = {mask.sum()} of {mask.size}")
    print("---")

print("Done. Next: python scripts/train.py --data-dir data --in-channels 3")
