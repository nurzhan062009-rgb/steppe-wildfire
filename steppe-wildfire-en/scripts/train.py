"""
Train a U-Net on Sentinel-2 patches for active-fire segmentation.

Demo: python scripts/train.py --synthetic --epochs 3 --in-channels 3
Real data: python scripts/train.py --data-dir data_patches --epochs 50 --in-channels 5 --save-path model.pth
"""
...
def get_source_group(patch_stem: str) -> str:
    """'kz_2022_09_03_p007' -> 'kz_2022_09_03', to group patches by source image."""
...
class WildfirePatchDataset(Dataset):
    ...
    def __init__(self, data_dir: str, in_channels: int = 3, crop_size: int = 480, augment: bool = False):
        ...
        if not self.files:
            raise FileNotFoundError(f"No .npy files in {self.image_dir}")
    ...
    def __getitem__(self, idx):
        ...
        # Sentinel-2 SR gives digital numbers 0-10000, convert to reflectance
...
class SyntheticWildfireDataset(Dataset):
    """Synthetic data for a pipeline sanity check without real imagery."""
...
def cross_validate(args):
    """Leave-one-source-out: each source image takes a turn as val, the rest as train."""
    ...
    print(f"Leave-one-source-out CV: {len(all_files)} patches from {len(groups)} sources, in_channels={args.in_channels}")
    ...
    for i, val_group in enumerate(groups):
        ...
        ious.append(iou)
        print(f"  [{i+1}/{len(groups)}] val_source={val_group} ({len(val_files)} patches): IoU={iou:.4f}")

    ious = np.array(ious)
    print(f"\n=== Leave-one-source-out CV result (in_channels={args.in_channels}) ===")
...
def train(args):
    # seed is fixed for weight init - otherwise run-to-run variance on a
    # small dataset masks the real difference between baseline and +NDVI/NBR
    torch.manual_seed(args.seed)
    np.random.seed(args.seed)
    ...
    if args.synthetic:
        ...
        # split by source image, not by individual patch - otherwise patches
        # from the same fire leak between train and val
        ...
        train_ds.files = train_files
        val_ds.files = val_files
        print(f"Train: {len(train_files)} patches from {len(groups)-n_val_groups} sources, "
              f"Val: {len(val_files)} patches from {n_val_groups} sources (seed=42)")
        print(f"Val sources: {sorted(val_groups)}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.patience > 0 and epochs_since_improvement >= args.patience:
            print(f"Early stopping: no improvement for {args.patience} epochs, stopping.")
            break

    print(f"\nBest val_IoU across training: {best_iou:.4f}")

    if args.save_path and best_state is not None:
        torch.save(best_state, args.save_path)
        print(f"Best model (not the last one!) saved to: {args.save_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--crop-size", type=int, default=480,
                         help="256 for pre-sliced patches, 480 for full images")
    ...
    if not args.synthetic and not args.data_dir:
        parser.error("Provide either --synthetic or --data-dir")

    if args.cross_val:
        if args.synthetic or not args.data_dir:
            parser.error("--cross-val only works with --data-dir")
        cross_validate(args)
    else:
        train(args)
