"""
U-Net for active-fire segmentation on multispectral Sentinel-2 imagery.

3 channels (SWIR2, SWIR1, NIR) - baseline, matching Pereira et al. 2021.
5 channels (+NDVI, +NBR) - extended version, see scripts/spectral_indices.py.
"""
...
class ConvBlock(nn.Module):
    """Two 3x3 conv layers + BatchNorm + ReLU + Dropout."""
...
class WildfireUNet(nn.Module):
    """
    U-Net for binary active-fire segmentation.

    in_channels: 3 = baseline (SWIR2, SWIR1, NIR), 5 = +NDVI/NBR
    base_filters: 32 by default, a lightweight setup for Colab
    """
    ...
    def forward(self, x):
        ...
        return self.outc(x)  # logits, sigmoid applied in the loss (BCEWithLogits)
